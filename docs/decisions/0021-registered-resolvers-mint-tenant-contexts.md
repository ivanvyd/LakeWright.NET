...
`AddLakeWrightTenancy<TResolver>()` creates that implementation and passes it only to an
`ITenantContextResolver` registration through `ActivatorUtilities`; it registers neither the
factory nor the concrete resolver type. The resolver must declare the constructor seam or
registration fails. The shipped EF resolver uses the same path. The worker's system-owned
resolution remains internal to the multitenancy assembly and uses that resolver rather than
restoring broad friend-assembly access.
...
- A controller, page, or background job cannot resolve the concrete registered resolver merely
  to reach a factory it retained.
