...
| Audit logging | CC7.2, CC7.3 | `audit_events` written in the same transaction as the audited action. Init-only entity, and every `SaveChanges`/`SaveChangesAsync` overload refuses a modify or delete. **Partial: `ExecuteUpdate`, `ExecuteDelete` and raw SQL bypass the change tracker and cannot be caught in C#.** Completing this needs `REVOKE UPDATE, DELETE ON audit_events` from the application role in the migration. Until then the guarantee holds for application code and not for the connection, and an auditor testing it at the database level would find the gap. |
