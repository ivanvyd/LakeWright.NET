...
Last checked 2026-08-06, when the release pipeline moved to keyless publishing. Checked 2026-08-05
before that, when three rows were found describing repository settings that had been enabled since,
and a control deferred to a milestone that had already shipped without it.
...
| Encryption in transit | CC6.7 | **Partial.** Databricks REST and SQL run over TLS by default, and that half needs no code. The ingress half (HTTPS only, HSTS, TLS 1.2 minimum) has nothing to configure yet because there is no ingress: nothing has been deployed. The scheduled TLS check that would evidence it arrives with the reference deployment, and not before, because there would be nothing for it to check. |
...
| Secret management | CC6.1, CC6.6 | No long-lived Databricks credentials: managed identity on Azure, OIDC federation in CI. ADR 0006, proved end to end in [spike 04](../planning/spike-04-managed-identity.md). Publishing to nuget.org uses trusted publishing rather than a stored key: a GitHub OIDC token is exchanged for one valid for an hour. Evidence: the absence of secrets in configuration and a clean history scan, plus secret scanning and push protection, both enabled on 2026-08-01 when the repository went public. |
...
| Data retention and deletion | Privacy, Confidentiality | **Implemented.** `TenantLifecycle` runs tenant deletion in the order documented by [data-handling.md](data-handling.md); `TenantLifecycleTests` covers both refusals and the successful purge. Audit retention is separately enforced by migration-role-only monthly partitions: the default is seven years, only partitions wholly before the cutoff are dropped, and the application role has no DDL. `AuditPartitionTests` covers populated migration, ACL and row-security preservation, retention boundaries, idempotency, rollback, and role separation. Scheduling and alerting the supplied maintenance command remain the adopter's operational responsibility. |
