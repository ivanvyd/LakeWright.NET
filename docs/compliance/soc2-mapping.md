...
|---|---|---|
| Tenant isolation | CC6.1, CC6.3 | Query layer that cannot build a statement without a resolved tenant context. Evidence: the cross-tenant isolation suite as a required CI check, with results retained per run. |
...
| Audit logging | CC7.2, CC7.3 | Append-only `audit_events` table written in the same transaction as the audited action. No update or delete path exists in code. Evidence: the table, plus a test asserting no mutation path. |
