...
|---|---|---|---|---|---|
| **A. Shared table + `tenant_id`** | .NET query layer | Table size only | Weakest. One missing predicate leaks everything. | Trivial | Lowest, one warehouse |
...
**Model A, shared table**, documented as the right answer for tenants counted in tens of thousands,
where B's schema ceiling and provisioning cost dominate. The isolation weakness is then handled by
making the query layer the only path to data and testing it adversarially.
