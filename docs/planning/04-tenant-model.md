...
|---|---|---|---|---|---|
| **A. Shared table + `tenant_id`** | A source-owned query layer or server-side row policy | Table size only | Not supported by LakeWright's generic SQL API: caller SQL cannot prove ownership of projected keys. | Trivial | Lowest, one warehouse |
...
**Model A, shared table**, can be appropriate for tenants counted in tens of thousands, where B's
schema ceiling and provisioning cost dominate. It needs a source-owned query layer or a server-side
row policy; LakeWright's generic statement API intentionally rejects it because a caller-projected
tenant key is not proof of ownership (ADR 0022).
