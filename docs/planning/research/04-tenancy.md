# 04 â€” Multi-Tenant Isolation on Databricks
...
**Status legend**
- **[VERIFIED]** â€” read from a primary source this session, URL given.
- **[COMMUNITY]** â€” practitioner write-up, blog, or forum. Not official.
- **[UNDETERMINED]** â€” looked for it, could not find a documented answer.
...
> "Delta Sharing is now OpenSharing" â€” Databricks blog, *Introducing OpenSharing: the Next Evolution of Delta Sharing for the Agentic Era*, 2026-06-10.
...
**Partner Well-Architected Framework (PWAF)** â€” launched 2026-02-10. [VERIFIED]
...
- Framework site: https://databrickslabs.github.io/partner-architecture/ â€” self-described as "The definitive architecture guide for technology partners to create world-class products, integrations, and data shares on the Databricks Data Intelligence Platform."
...
**Older ISV best-practices PDF** â€” https://assets.docs.databricks.com/_extras/documents/best-practices-building-isv-integrations.pdf
Title metadata reads `[For External] Best Practices - Building ISV Integrations - V(2.6.1)`, dated **2024-06-19**. Could not extract body text (binary PDF, no local PDF renderer available in this environment). Given the 2024 date it predates ABAC policies, query tags, OAuth token federation and Databricks Apps user authorization â€” treat as stale and do not cite it for mechanics.
...
> "**Tenant isolation** â€” Restricts rows to the user's organization: `CREATE POLICY tenant_data ON clients USING (tenant_id = (SELECT tenant_id FROM user_tenants WHERE user_email = current_user));`"
> â€” https://docs.databricks.com/aws/en/oltp/projects/data-api (doc date 2026-07-13)
...
Note the same trap applies: Lakebase RLS keys off the Postgres `current_user`, which is the Postgres role mapped to the **authenticated Databricks identity**. One shared service principal â‡’ one role â‡’ no per-tenant discrimination. Same failure mode as Unity Catalog row filters (see Â§3).

### 1.3 The closest thing to official isolation guidance

**Unity Catalog best practices** â€” https://learn.microsoft.com/en-us/azure/databricks/data-governance/unity-catalog/best-practices (doc date **2026-07-27**). Verbatim:
...
**Workspace-catalog binding** â€” https://learn.microsoft.com/en-us/azure/databricks/data-governance/unity-catalog/access-control/workspace-catalog-binding (doc date **2026-04-29**, updated 2026-05-12):
...
**Workspaces per account** â€” the AWS limits page lists it, the Azure page does not (on Azure a workspace is an ARM resource, so the constraint moves to Azure subscription quotas: 980 resource groups per subscription, 800 resources per resource group, 25,000 VMs per subscription per region).
- AWS: **Workspaces (Premium tier) = 10 per account**, **Workspaces (Enterprise tier) = 50 per account**, Fixed: No. â€” https://docs.databricks.com/aws/en/resources/limits
...
**OpenSharing (formerly Delta Sharing) limits** â€” same page:
...
**API rate limits that constrain per-tenant provisioning** â€” same page:
...
**Quota introspection APIs** â€” https://docs.databricks.com/aws/en/data-governance/unity-catalog/resource-quotas (doc date 2026-06-03):
- `GET /api/2.1/unity-catalog/resource-quotas/{parent_securable_type}/{parent_full_name}/{quota_name}` â€” "Returns accurate counts within 30 minutes of creation operations."
- `GET /api/2.1/unity-catalog/resource-quotas/all-resource-quotas` â€” "Unlike `GetQuotas`, `ListQuotas` has no SLA on the freshness of counts."
...
**Mechanics** [VERIFIED] â€” https://learn.microsoft.com/en-us/azure/databricks/data-governance/unity-catalog/filters-and-masks/ (doc date **2026-07-30**) and `.../manually-apply` (doc date **2026-07-21**).
...
**Verdict:** scales to any tenant count on object counts (one table, one filter). Isolation strength depends entirely on the caller identity question in Â§3.
...
**Ceiling:** **10,000 schemas per catalog** (raisable). **10,000 tables per schema.** The grant ceiling is the sharper constraint: **4,000 privileges on a parent object** (metastore, catalog, schema). If every tenant's grants land on the shared catalog, you hit 4,000 rows of grants long before 10,000 schemas â€” budget grants per catalog, not schemas.

**Correction to a widely-cited claim:** DevIQ's Part 1 states schemas have *"no hard limits on number"* (https://www.deviq.io/insights/multi-tenant-isolation-databricks-part-1, Shawn Davison, 2026-04-21). That is **wrong** against the current limits page â€” 10,000 per catalog, marked `Fixed: No`. Don't repeat it in the planning package.

**Cost:** no per-schema charge. Storage can be set at schema level (managed storage is resolved at the lowest available level in metastore â†’ catalog â†’ schema).
...
**Ceiling:** **1,000 catalogs per metastore** (`Fixed: No`, raisable via account team). This is the hard wall for this model â€” 1,000 tenants out of the box, and you must negotiate for more.
...
**Cost:** no per-catalog charge. Real cost is operational â€” provisioning, grant management, and the SCIM `PATCH 2/s` rate limit during onboarding.

---

### 2.d Workspace-per-tenant

**Ceiling â€” this is the model that dies first.**
- AWS: **10 workspaces per account (Premium)**, **50 (Enterprise)**, raisable. â€” https://docs.databricks.com/aws/en/resources/limits
- Azure: no documented per-account workspace cap; constrained instead by Azure subscription quotas and by **50 workspaces per network connectivity configuration** with **10 NCCs per region per account** (â‡’ ~500 workspaces per region if every workspace needs serverless private networking).

**Isolation strength:** highest â€” separate control plane surface, separate compute, separate workspace-local admin groups, and a natural blast radius boundary.
...
**Mechanics** â€” https://docs.databricks.com/aws/en/opensharing/ (doc date **2026-07-20**):
...
**Ceiling:** **5,000 recipients per metastore**, **1,000 shares per metastore**, and critically **20 shares per catalog** and **1,000 tables per share**. If you model one share per tenant off a single shared catalog, the **20-shares-per-catalog** limit bites at 20 tenants. You'd need ~1 catalog per 20 tenants, re-coupling you to the 1,000-catalog cap (â‡’ ~20,000 tenants theoretical, but you've now got catalog sprawl).
...
## 3. Row-level security mechanics â€” and the service-principal question
...
> â€” `current_user` reference, doc date 2026-06-16
...
> â€” https://learn.microsoft.com/en-us/azure/databricks/data-governance/unity-catalog/filters-and-masks/manually-apply, doc date 2026-07-21

And for ABAC policies:

> Policies on underlying tables "are evaluated using the **session user's identity**" â€” the person (or principal) running the query.
> â€” https://docs.databricks.com/aws/en/data-governance/unity-catalog/abac/requirements
...
2. When the connection is authenticated as a service principal, `session_user()` returns **that service principal's UUID** â€” documented verbatim above. It does not, and cannot, know which of your end users triggered the HTTP request.
...
**Conclusion: with a single shared service principal you MUST enforce tenancy in your own query layer.** Row filters degrade to a defence-in-depth backstop against your own bugs at best â€” and even that is weak, because the only correct configuration for a shared SP is "SP sees everything", which means the filter contributes nothing.
...
> â€” https://learn.microsoft.com/en-us/azure/databricks/dev-tools/databricks-apps/auth, doc date **2026-07-21**
...
That is the official, explicit statement of the trade-off: **app identity â‡’ you filter; user identity â‡’ Unity Catalog filters.**
...
1. **Per-tenant service principal** â€” the SP is the tenant. `session_user()` = SP UUID = tenant key. Filter on a mapping table keyed by SP UUID, or put each SP in a `tenant_<id>` group and use `is_account_group_member()`. Ceiling: **10,000 users + service principals per account, combined** (`Fixed: No`).
2. **On-behalf-of end-user tokens** â€” the connection is the end user. Ceiling: end users must exist in your Databricks account, so the same 10,000 cap applies, and it's now shared with real humans.
3. **Per-tenant warehouse/compute with a per-tenant run-as identity** â€” same as (1) with extra cost.

### 3.4 Compute support

| Compute | Table-level filters/masks | ABAC policies |
|---|---|---|
| **SQL warehouse** (classic/pro/serverless) | âœ… Supported | âœ… Supported |
| Serverless compute | âœ… | âœ… |
| Standard access mode | âœ… DBR 12.2 LTS+ | âœ… DBR 16.4+ |
| Dedicated access mode | âš ï¸ DBR 15.4 LTS+ read-only; writes DBR 16.3+; runs FGAC on serverless (billable) | âš ï¸ DBR 16.4+ with FGAC |
| DBR < 12.2 LTS | âŒ fails securely (returns no data) | âŒ |
| Iceberg REST catalog / Unity REST API | âŒ | âŒ |
| Delta Lake APIs | âŒ | âŒ |

Source: filters-and-masks index + manually-apply + abac/requirements, all cited above. **SQL warehouses (including serverless) are fully supported** â€” which is what a .NET app connecting over the SQL connector or Statement Execution API will use.

---

## 4. Acting as the end user

### 4.1 Databricks Apps on-behalf-of-user (OBO) â€” Public Preview
...
- Scopes are declared per app (`sql`, `genie`, `files`, â€¦). Default without any scope: `iam.access-control:read`, `iam.current-user:read` â€” identity only, no data.
...
**Applicability to Lakewright.NET:** this is the *Databricks-hosted* app runtime. An externally-hosted ASP.NET app does not receive `x-forwarded-access-token`. To replicate the pattern outside Databricks Apps you need Â§4.2.

### 4.2 OAuth token federation â€” the external-app path
...
Two policy types:
- **Account federation policy** â€” "enables all users and service principals in your Databricks account to access Databricks APIs using tokens from your identity provider." Typically paired with SCIM so IdP users are synced into the account.
- **Service principal federation policy** (workload identity federation) â€” an automated workload authenticates **as a specific service principal** using a runtime-issued token, no stored Databricks secret.
...
- Rate limits on the exchange endpoint: **[UNDETERMINED]** â€” not documented.

**The catch:** the app cannot mint identities. It can only exchange a JWT that its IdP already issued for a subject that already exists in the Databricks account. So a true per-end-user model requires **every end user of every tenant to be SCIM-provisioned into your Databricks account** â€” against the **10,000 users + service principals** cap. For a SaaS with 1,000 tenants Ã— 20 users, that's 20,000 principals: over the cap, needing an account-team increase, plus SCIM `PATCH 2/s` for the churn.

### 4.3 Per-tenant service principals â€” the pragmatic middle

Cost and limits:
- **10,000 users + service principals per account, combined, `Fixed: No`** (raisable). â€” resources/limits, 2026-07-28
- **5,000 groups per account, `Fixed: Yes`** â€” so a `tenant_<id>_users` group per tenant caps you at 5,000 tenants and that one is marked non-raisable.
...
> â€” Ust Oldfield (Head of Analytics, Advancing Analytics), *Built-On Databricks: Delivering Multi-Tenant Analytics*, **2026-05-20**. https://www.advancinganalytics.co.uk/blog/built-on-databricks-delivering-multi-tenant-analytics
...
### 4.4 `SET` session variables â€” not a security boundary

**[UNDETERMINED as a security mechanism, and do not use it as one.]** There is no documented "impersonate this user for this session" `SET` command. Query tags (Â§5) are session/statement metadata for attribution, explicitly warned as plain text and globally replicated â€” they are not authenticated and any client can set any value. Anything that reads a client-supplied session variable inside a row filter is trivially spoofable by whoever holds the connection.

---

## 5. Cost attribution per tenant

### 5.1 Query tags â€” the right primitive (Public Preview)
...
- **Statement Execution API** (statement-level) â€” the cleanest fit for an ASP.NET backend:
  ```bash
...
- "Query tags are limited to **10 KB per session**." Exceeding â‡’ tags dropped + `tags_dropped: true` sentinel.
- "Each query supports a maximum of **20 user-specified tags**."
- "Tag keys and values must not exceed **128 characters**."
- "Tag keys must not contain the characters `,`, `:`, `-`, `/`, `=`, or `.`" â€” **so `tenant-id` and `tenant.id` are invalid keys; use `tenant_id`.**
...
â‡’ Use an opaque tenant GUID, never a customer name.
...
Key columns: `record_id`, `workspace_id`, `sku_name`, `usage_start_time`/`usage_end_time`/`usage_date`, **`custom_tags` (map)**, `usage_quantity` (DBUs), **`usage_metadata` struct** (`warehouse_id`, `cluster_id`, `job_id`, `job_run_id`, `app_id`, `app_name`, `dlt_pipeline_id`, `endpoint_name`, `usage_policy_id`, â€¦), **`identity_metadata` struct** (`run_as`, `owned_by`, `created_by`), `record_type` (`ORIGINAL` / `RETRACTION` / `RESTATEMENT`), `billing_origin_product`, `product_features`.

**`identity_metadata.owned_by`** "only applies to SQL warehouse usage and logs the user or service principal who **owns** the SQL warehouse responsible for the usage" â€” so warehouse-per-tenant gives you first-class billing attribution via `owned_by` + `warehouse_id`.

**Correction handling matters** â€” always aggregate, never read single rows:
```sql
...
|---|---|---|---|
| **Warehouse-per-tenant** | Direct $ | Exact | Tag the warehouse with `tenant_id` â‡’ flows to `usage.custom_tags`; also `usage_metadata.warehouse_id` + `identity_metadata.owned_by` |
...
### 5.5 Latency â€” this is the answer to "what's the latency of system tables"
...
> â€” https://learn.microsoft.com/en-us/azure/databricks/admin/system-tables/ (doc date **2026-07-30**), "Known issues"
...
- **`VACUUM` retention is the default 7 days** â€” "your streaming query might break if it lags behind by more than 7 days."
...
- "New columns may be added to existing system tables at any time" â€” enable schema evolution if you copy them.
...
### 6.1 Official â€” concurrency, queuing, cold start

**SQL warehouse sizing, scaling and queuing** â€” https://learn.microsoft.com/en-us/azure/databricks/compute/sql-warehouse/warehouse-behavior (doc date **2026-07-21**). Verbatim:
...
**Databricks blog, *Architecting a High-Concurrency, Low-Latency Data Warehouse on Databricks That Scales*** â€” 2025-09-02, Ben Dunmire, Dan Lueck, Jen Lim. https://www.databricks.com/blog/architecting-high-concurrency-low-latency-data-warehouse-databricks-scales
- "Segment users (human/automated) and their query patterns (interactive BI, ad hoc, scheduled reports) to use different warehouses scoped by application context." â€” i.e. **Databricks' own answer to noisy neighbours is warehouse segmentation**, which for a SaaS means warehouse-per-tier or warehouse-per-tenant-cohort.
- Threshold: "persistent queuing (where **peak queued queries are >10**)" indicates a scaling problem.
- On noisy neighbours specifically: "**Reach out to your Databricks account contact to access upcoming features intended to prevent noisy neighbors.**" â‡’ As of that post there was **no shipped, self-serve noisy-neighbour control**. I found no later doc superseding this. **[UNDETERMINED]** whether such a feature has since GA'd.
...
- *Caveat:* that answer predates or sits alongside the OAuth token federation docs (2026-01-29 / 2026-06-16), which do allow an arbitrary HTTPS issuer + JWKS. Whether account-wide token federation now supersedes the "must transit through Entra" answer on Azure is **[UNDETERMINED]** â€” worth a direct test before building on it.
...
- **Says nothing about cost attribution.** That absence is itself informative â€” the hard part is under-served in community material.
...
- âš ï¸ **Contains a factual error**: claims schemas have "no hard limits on number". Contradicted by the limits page (10,000 per catalog). Don't cite this number.
- Part 2, *Tenant-Scoped Genie Spaces*, **2026-05-06** (https://www.deviq.io/insights/multi-tenant-isolation-databricks-part-2) argues for one Genie space per tenant â€” relevant because **Genie Agents are limited to 30 tables or views per space** (`Fixed: No`) and there is no documented per-account Genie space cap, but 10,000 conversations per space.

**[COMMUNITY] App-layer-owns-tenancy, SPN-per-tenant.** Ust Oldfield, Advancing Analytics, **2026-05-20** (cited in Â§4.3). Also warns against "recreating Databricks UI features", which results in "maintaining a poor imitation".
...
- XL warehouse, 1â€“8 cluster autoscale, ~$150/day, **45-minute auto-stop**, provisioned for a 12â€“15 concurrent-query peak while actual average was 3â€“5. 23 queries ran >15 minutes. Reduced $17,500 â†’ $15,000/month.
- Two lessons for us: **auto-stop is the single biggest idle-cost lever**, and **you need a per-query kill switch** â€” which is what the Beta warehouse-level `statement_timeout` provides.

**[COMMUNITY] Provisioning throughput.** Databricks Community threads on ingesting from hundreds of separate ADLS accounts note that each external location maps to exactly one storage path with no wildcards, and flag "no granular permissions at the metastore level â€” you're either an admin or not", which complicates having a provisioning service principal that can create and grant across all tenant catalogs without being a full metastore admin. https://community.databricks.com/t5/data-engineering/best-pattern-for-ingesting-data-from-hundreds-of-separate-adls/td-p/149991

### 6.3 Pitfalls I'd add from reading the primary docs

1. **The ANSI-mode row-filter footgun** (Â§2.a) â€” a type mismatch silently returns *all rows*. Highest-severity finding in this document.
...
3. **Materialized views and streaming tables break under ABAC** unless the pipeline owner and run-as identity are in `EXCEPT` â€” a refresh-failure trap that only appears in production.
...
8. **Workspace-local admin groups don't cross workspaces** â€” bites when unbinding or re-binding default workspace catalogs.
...
2. **Do not build the isolation story on row filters with a shared service principal.** Cite Â§3.2. If we want UC-enforced tenancy, we need per-tenant identity on the connection, and that is a deliberate, costly architectural choice with a 10,000-principal ceiling.
3. **Row filters/ABAC as defence-in-depth only where a per-tenant SPN exists** â€” where `session_user()` is genuinely the tenant.
...
- **[UNDETERMINED]** Metastore-wide schema cap â€” is there one distinct from the 10,000-per-catalog cap?
