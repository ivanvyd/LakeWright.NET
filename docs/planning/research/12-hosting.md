# Lakewright.NET â€” Hosting & Reference Deployment

Research date: **2026-07-31**. All pricing observed **2026-07-31, region East US, currency USD**, unless stated.

Evidence labels used throughout:
- **[V]** VERIFIED â€” stated explicitly in a primary source, cited.
- **[V-COMP]** VERIFIED BY COMPOSITION â€” each half documented in a primary source, cited; the combination was not executed in this session.
- **[R]** RECALLED / secondary source â€” treat as a lead, not a fact.
...
| Reference deployment cost | **$0â€“1/month** demo (scale-to-zero, inside the ACA free grant); **~$11/month** if kept always-warm. |
...
ACA does not have a ".NET version" â€” it runs containers. **[V]**
...
> â€” https://learn.microsoft.com/azure/container-apps/containers

Constraints that matter for a .NET image: **[V]**
- `linux/amd64` required. **An `arm64`-only image will not run.** This matters because Apple Silicon and Windows-on-ARM developers will produce arm64 images by default â€” the Dockerfile/CI must pin `--platform linux/amd64` or build a multi-arch manifest.
...
`0.25/0.5Gi`, `0.5/1.0Gi`, `0.75/1.5Gi`, `1.0/2.0Gi`, â€¦ up to `4.0/8.0Gi`.

> "Apps using the Consumption plan in a *Consumption only* environment are limited to a maximum of 2 cores and 4Gi of memory."
> â€” https://learn.microsoft.com/azure/container-apps/containers#configuration
...
> â€” https://learn.microsoft.com/azure/container-apps/billing
...
> **Planning trap.** The "<0.01 vCPU" condition is easy to violate accidentally. A background `IHostedService` doing a periodic poll, a health-check timer, an OpenTelemetry metrics exporter on a 15s interval, or an EF Core connection-pool keepalive can each push a nominally-idle ASP.NET Core app over 0.01 vCPU and silently move the whole month to the **8Ã— more expensive active rate**. If Lakewright.NET ships background jobs on by default, the always-warm cost estimate below is wrong. Either gate background work behind a flag that the demo profile disables, or budget at active rates.

Cold start guidance â€” Microsoft publishes mitigation advice but **no latency SLO or published number**: **[V]**
...
> â€” https://learn.microsoft.com/azure/container-apps/cold-start
...
Third-party measurements found (**[R]** â€” blogs, not primary, and not reproduced here): 1â€“3 s for small containers, 5â€“10 s typical for .NET, and 15â€“30 s for a .NET app with a large DI graph. Treat these as order-of-magnitude only.
...
Both system-assigned and user-assigned identities are supported. **[V]** â€” https://learn.microsoft.com/azure/container-apps/managed-identity

Known limitation: init containers cannot access managed identities in consumption-only environments and dedicated workload profile environments. **[V]** (Irrelevant here unless migrations move to an init container â€” worth knowing, because "run EF Core migrations in an init container" is the obvious pattern and it would break MI-based DB auth.)

Mechanism â€” ACA injects two environment variables and exposes a local token endpoint: **[V]**
- `IDENTITY_ENDPOINT` â€” local URL to request tokens from.
- `IDENTITY_HEADER` â€” anti-SSRF header value, rotated by the platform.
...
> â€” https://learn.microsoft.com/azure/container-apps/managed-identity#rest-endpoint-reference (emphasis added)

This is the hinge for Â§5. For .NET the recommended abstraction over this endpoint is `Azure.Identity` / `DefaultAzureCredential`. **[V]**

> "When using Azure Identity client library, you need to explicitly specify the user-assigned managed identity client ID."

Also worth adopting: `identitySettings.lifecycle` (API `2024-02-02-preview`+) restricts which containers can use which identity â€” `Init` / `Main` / `All` / `None`. An ACR-pull identity should be `None` so application code cannot borrow it. **[V]**
...
Every container app with external HTTP ingress gets an auto-generated FQDN with TLS at no cost. Custom domains get a **free managed certificate**, auto-renewed. **[V]** â€” https://learn.microsoft.com/azure/container-apps/custom-domains-managed-certificates

Requirements: **[V]**
- HTTP ingress enabled and publicly reachable from the DigiCert validation IPs.
- Apex domain â†’ `A` record to the environment's static IP, plus `TXT` at `asuid`.
- Subdomain â†’ `CNAME` to the app's generated FQDN, plus `TXT` at `asuid.<sub>`.
...
Note the active/idle spread is **8Ã— on vCPU** and **1Ã— on memory** (memory active and idle are the same rate, $0.000003/GiB-s). Idle mode therefore only saves money on CPU.
...
> â€” https://learn.microsoft.com/azure/container-apps/billing#consumption-plan
...
#### Scenario A â€” scale-to-zero demo (recommended)
...
| --- | --- | --- | --- |
| vCPU-seconds | 0.5 Ã— 216,000 = **108,000** | under 180,000 | **$0.00** |
| GiB-seconds | 1.0 Ã— 216,000 = **216,000** | under 360,000 | **$0.00** |
...
#### Scenario B â€” always-warm (`minReplicas: 1`), same size

730 h/month = 2,628,000 s total; assume 60 h active, 670 h idle. Assumes the app genuinely stays under 0.01 vCPU when idle (see the Â§1.3 trap).

| Line | Quantity | Billable after grant | Rate | Cost |
| --- | --- | --- | --- | --- |
| vCPU-s active | 108,000 | 0 (grant) | â€” | $0.00 |
| vCPU-s idle | 1,206,000 | 1,134,000 | $0.000003 | **$3.40** |
| GiB-s active | 216,000 | 0 (grant) | â€” | $0.00 |
| GiB-s idle | 2,412,000 | 2,268,000 | $0.000003 | **$6.80** |
| Requests | <2M | 0 | â€” | $0.00 |
| **ACA compute total** | | | | **â‰ˆ $10.20 / month** |

(180,000 vCPU-s and 360,000 GiB-s of grant applied to active usage first, remainder against idle.)

If the idle conditions are *not* met â€” background jobs keep CPU above 0.01 vCPU â€” the same workload bills at active rates: vCPU 1,314,000 âˆ’ 180,000 = 1,134,000 Ã— $0.000024 = **$27.22** plus memory $6.80 = **â‰ˆ $34/month**. That is a 3.3Ã— miss, and it is the realistic outcome for an app with a default-on background scheduler.

#### Ancillary costs not in the meters above

- **Log Analytics workspace** â€” an ACA environment sends logs to Log Analytics (or Azure Monitor). A demo app's volume is small; estimate well under $1/month. **[R]** â€” not priced via the API in this session.
- **Container registry** â€” use **GitHub Container Registry (`ghcr.io`)**, free for public images, rather than Azure Container Registry. This removes a cost line *and* is the correct choice for an OSS project (contributors can pull without an Azure account). Note the Â§1.3 tradeoff: a registry outside Azure adds image-pull latency to cold start.
- **Virtual network** â€” not used in the reference deployment. ACA warns that BYO-vnet adds charges. **[V]**

### 1.7 `azd` and .NET Aspire

`azd` (Azure Developer CLI) has first-class Aspire support: it detects an Aspire app host and provisions a resource group, Azure Container Registry, Log Analytics workspace, and Container Apps environment, then deploys. **[V]** â€” https://learn.microsoft.com/dotnet/aspire/deployment/azure/aca-deployment-visual-studio

Aspire 9.2+ adds `aspire publish` / `aspire deploy` with extensible publishers; for Azure specifically, `azd` remains the recommended path. **[R]** â€” https://aspire.dev/deployment/azure/container-apps/ (vendor doc, but secondary to Learn for version specifics; the 9.2 version boundary was not independently confirmed).

**Recommendation for Lakewright.NET: adopt Aspire for local orchestration (F5 â†’ app + Postgres + seeded Databricks config), but do NOT make `azd`/Aspire the only deployment path.** Reasons:
...
### 2.1 Free (F1) tier â€” not viable for a credible demo
...
| CPU quota | **60 CPU-minutes per day**, shared per region per subscription across all Free apps. On exhaustion the app is stopped with `Error 403 â€“ Web app is stopped (Quota exceeded)`. **[R]** |
| Price | $0.00/hour (`F1 App`, Azure App Service Free Plan - Linux, East US, 2026-07-31). **[V]** |

The 60 CPU-min/day quota is the killer: a demo that gets shared on Hacker News or LinkedIn will hard-stop partway through the day and stay down. The `*.azurewebsites.net` hostname does get TLS, so F1 is usable for a *private* scratch deploy â€” but not for a public demo.
...
### 2.2 Paid tiers â€” real prices

Azure Retail Prices API, `serviceName eq 'Azure App Service'`, Linux products, East US, USD, retrieved **2026-07-31**. Monthly = hourly Ã— 730.
...
> â€” https://docs.databricks.com/aws/en/dev-tools/databricks-apps/

No container or bring-your-own-image option is documented. The `app.yaml` contract is a `command` plus `env` executed inside a Databricks-managed base image; there is no published mechanism for supplying your own image or installing a .NET runtime. **[V]** â€” https://docs.databricks.com/aws/en/dev-tools/databricks-apps/app-runtime

### 3.2 Blocker 2 â€” cannot serve external customers (decisive)
...
> â€” https://docs.databricks.com/aws/en/dev-tools/databricks-apps/permissions
...
> â€” https://docs.databricks.com/aws/en/dev-tools/databricks-apps/auth

The broadest possible grant is `All account users` â€” "All users and service principals in the current Databricks account." **[V]** External collaborators must be onboarded into your identity provider via SCIM/JIT provisioning, i.e. they become identities in *your* Databricks account. **[V]**
...
| eShopOnContainers (archived; â†’ `dotnet/eShop`) | Docker Compose + Helm charts for AKS **[R]** |
| fullstackhero/dotnet-starter-kit (.NET 10) | Docker Compose **[R]** |
| boxyhq/saas-starter-kit | `docker-compose.yml` **[R]** |

Sources: https://abp.io/docs/latest/solution-templates/microservice/helm-charts-and-kubernetes, https://github.com/dotnet-architecture/eShopOnContainers, https://github.com/fullstackhero/dotnet-starter-kit, https://github.com/boxyhq/saas-starter-kit. These are secondary/vendor pages surveyed at a distance, not audited repo-by-repo â€” labelled **[R]**.

The pattern: **Docker Compose is universal; Helm appears only in microservices-shaped projects.** ABP and eShop ship Helm because they have 8â€“15 services to orchestrate. A single-service accelerator does not clear that bar.

### 4.2 The honest minimum

**Ship a `Dockerfile` and a `docker-compose.yml`. Do not ship a Helm chart at launch.**

- **`Dockerfile`** â€” multi-stage, `mcr.microsoft.com/dotnet/sdk:10.0` â†’ `mcr.microsoft.com/dotnet/aspnet:10.0-noble-chiseled`, built `--platform linux/amd64` (see Â§1.1). This one file *is* the portability story: it is what ACA runs, what Cloud Run runs, what ECS runs, and what a Kubernetes `Deployment` runs.
- **`docker-compose.yml`** â€” app + Postgres, so `docker compose up` gives a working local instance with no Azure account. This is the contributor on-ramp and matters more than any cloud deployment target.
- **`infra/main.bicep`** â€” the Azure reference deployment. Explicitly labelled as *a* reference, not *the* deployment.

A Helm chart written before a single user has asked for one is a maintenance liability: it must track every app config change, it cannot be meaningfully tested in CI without a cluster, and a stale chart is worse than no chart. Add it when someone opens an issue asking for it â€” and note that Databricks' own workload-identity-federation docs give Kubernetes a **documented, secret-free** auth path (Â§5.4), so the Helm story is technically strong whenever it is written.

What makes the "runs anywhere" claim credible is not the number of manifests â€” it is that **the app takes all configuration from environment variables and acquires credentials through a pluggable provider** (Â§5.5). Ship that, and one Dockerfile is a complete portability story. Ship five manifests around a hard-coded `DefaultAzureCredential`, and it is not.

---

## 5. Managed identity â†’ Databricks (the key security question)
...
> â€” https://learn.microsoft.com/en-us/azure/databricks/dev-tools/auth/aad-token-manual
...
> â€” https://learn.microsoft.com/en-us/azure/databricks/dev-tools/auth/azure-mi-auth
...
3. Add the MI to the Databricks **account** as a service principal â€” choose **Microsoft Entra ID managed** and paste the MI's **Client ID** as the Microsoft Entra application ID. **[V]** (azure-mi-auth Step 2)
...
**No client secret anywhere.** **[V-COMP]** â€” Each half is documented: ACA's token endpoint accepts "any other resource URI" (Â§1.4) and `2ff814a6-â€¦` is the documented Databricks resource ID accepted as a bearer token (Â§5.1). The `.default` scope form is documented for MSAL against this exact resource. The composition (ACA MI â†’ Databricks REST) was **not executed in this session** and should be smoke-tested before it goes in the README as a claim.

Two setup notes that will cost time if missed:
- `ManagedIdentityClientId` is **mandatory** for user-assigned identities â€” `DefaultAzureCredential` otherwise tries the system-assigned identity. **[V]** (Â§1.4)
- After step 5, the ~24h managed-identity token cache (Â§1.4) can serve stale permissions. Restart the revision.
...
> â€” https://learn.microsoft.com/en-us/azure/databricks/dev-tools/auth/azure-mi
...
> "If unspecified, Azure Databricks retrieves the keys from the issuer's well-known endpoint, which is the recommended approach. Your identity provider must serve OpenID Provider Metadata at `<issuer-url>/.well-known/openid-configuration` that includes a `jwks_uri`â€¦"
> â€” https://learn.microsoft.com/en-us/azure/databricks/dev-tools/auth/oauth-federation-policy
...
| --- | --- | --- |
| Azure Databricks + Azure Container Apps | Entra token for `2ff814a6-â€¦`, direct bearer | **[V]** documented end to end |
| Any Databricks + **Kubernetes** (EKS/GKE/AKS/on-prem) | SA token â†’ federation policy | **[V]** documented |
| **AWS** Databricks + Lambda/ECS/EC2 | AWS IAM outbound identity federation | **[V]** documented |
| **GCP** Databricks + Cloud Run/GCE | No GCP-specific example published | **[R]** â€” GKE works via the Kubernetes row; native Cloud Run/GCE was **not** confirmed. Do not claim it. |
| Non-Azure Databricks + **Azure** compute (Entra as issuer) | Plausible: register `https://login.microsoftonline.com/<tenant>/v2.0` as issuer | **INFERRED, NOT VERIFIED** â€” Entra is a conforming OIDC issuer and the policy accepts arbitrary issuers, but this combination is not in Databricks' documented examples and was not tested. Do not ship as a claim. |
...
â”œâ”€â”€ EntraManagedIdentityProvider   // Azure: DefaultAzureCredential â†’ 2ff814a6-â€¦/.default   [V]
â”œâ”€â”€ OidcFederationProvider         // K8s SA token / AWS IAM / CI OIDC â†’ Databricks token exchange   [V]
â””â”€â”€ PersonalAccessTokenProvider    // local dev only; loud warning if used outside Development
```
...
| --- | --- | --- |
| **0 â€” best** | **No secret at all** â€” managed identity / workload identity federation (Â§5) | Databricks, Azure SQL, Key Vault, Storage |
| 1 â€” local dev | **.NET user-secrets** (`dotnet user-secrets`) | Dev-only PATs, local connection strings. Never in the repo. |
| 2 â€” deployed, unavoidable secrets | **Azure Key Vault**, referenced from Container Apps secrets, read via managed identity | Third-party API keys, SMTP, Stripe |
| 3 â€” config, not secrets | Plain environment variables | Workspace URL, warehouse ID, catalog name, feature flags |
| â€” avoid | Container Apps inline secret values in production | Explicitly discouraged by Microsoft |
| â€” not applicable | Databricks secret scopes | Scoped to code *running inside* Databricks (notebooks/jobs), not an external ASP.NET Core app |
...
Container Apps natively resolves Key Vault references â€” the app never sees the vault. **[V]** â€” https://learn.microsoft.com/en-us/azure/container-apps/manage-secrets
...
- URI **without** a version â†’ app picks up the latest version automatically **within 30 minutes**, and any active revision referencing it in an env var is **automatically restarted** to pick up the new value.
- URI **with** a version â†’ pinned; full control, manual rotation.

For an accelerator, omit the version so rotation is automatic â€” but document the auto-restart, because an unexplained restart during a demo is alarming.

Two operational notes: **[V]**
- **System-assigned identity cannot be used with `az containerapp create`** â€” it does not exist until after the app is created. Use a **user-assigned** identity, which also matches the Databricks recommendation in Â§5.1. This makes UAMI the correct choice on both counts.
...
Secrets can also be mounted as files in a volume (one file per secret) rather than env vars â€” useful for anything multi-line. **[V]**
...
| ACA always-warm (`minReplicas: 1`) | ~$10.20 | No cold start â€” if idle conditions hold (Â§1.3). |
...
| Fly.io | ~$2â€“25 | Free tier discontinued; trial is 2 VM-hours / 7 days. **[R]** |
| Hetzner + Coolify | ~$4â€“5 | Cheapest always-on, but it is a server you now operate. **[R]** |

Third-party pricing is **[R]** â€” aggregator blogs, not vendor pricing pages, and volatile. Re-check before publishing any of it.
...
1. **The live demo is not the bottleneck on adoption.** Someone evaluating a Databricks SaaS accelerator needs to see it against *their* Databricks workspace with *their* data. A shared public demo cannot show that â€” it can only show a UI, which a 90-second screen recording shows equally well at zero risk.
...
4. **The costs are asymmetric.** A recording costs an hour once. A live demo costs the always-warm bill plus the Databricks SQL warehouse behind it â€” which is the real expense and is *not* $10/month â€” plus ongoing operational attention.
...
- Backed by **synthetic seed data in Postgres**, not a live Databricks workspace â€” demonstrates the app; risks nothing.
- README states "first load takes a few seconds â€” the demo scales to zero," which turns the cold start from a defect into a demonstrated feature.
- Budget alert on the subscription at $20.

That version is genuinely ~$0â€“1/month and carries almost no risk. It is worth doing **after** the project has users, not before.
...
```
GitHub repo
 â”œâ”€â”€ src/                       ASP.NET Core 10 app
 â”œâ”€â”€ Dockerfile                 multi-stage â†’ aspnet:10.0-noble-chiseled, linux/amd64
 â”œâ”€â”€ docker-compose.yml         app + Postgres â€” the contributor on-ramp
 â”œâ”€â”€ infra/main.bicep           Azure reference deployment
 â””â”€â”€ .github/workflows/         build â†’ push ghcr.io â†’ az containerapp update
```
...
| --- | --- | --- |
| Container Apps environment | Consumption workload profile | $0 (no dedicated profile â†’ no plan management fee) |
| Container App | 0.5 vCPU / 1.0 GiB, `minReplicas: 0`, external HTTP ingress | **$0.00** (inside free grant) |
| User-assigned managed identity | â€” | $0 |
...
| **Total** | | **â‰ˆ $0â€“1 / month** |

Always-warm variant (`minReplicas: 1`): **â‰ˆ $11/month**, subject to the Â§1.3 idle-conditions caveat.

**Auth posture:** user-assigned managed identity â†’ Entra token for `2ff814a6-3304-4ab8-85cb-cd0e6f879c1d` â†’ Databricks REST API. Zero stored Databricks credentials. Key Vault for third-party secrets only, resolved by the same identity.

**Why this shape:**
- The unit of deployment is an OCI image, so ACA / Kubernetes / Compose / ECS are all the same artifact â€” the cloud-neutrality constraint is satisfied structurally, not by paperwork.
...
1. Smoke-test the ACA-MI â†’ Databricks REST call end to end (Â§5.2) â€” documented on both sides, not executed here.
...
4. Do not assert GCP Cloud Run or Entra-as-federation-issuer support (Â§5.4) without testing.
