# 06 â€” Deploying Databricks assets as code, and the CI/CD story
...
## 1. NAMING â€” the headline finding
...
â€” <https://docs.databricks.com/aws/en/dev-tools/bundles/> (page last updated **Jul 10, 2026**)
...
â€” <https://learn.microsoft.com/en-us/azure/databricks/release-notes/dev-tools/bundles>
...
entry cites **no CLI version at all** â€” it is a documentation/branding change, not a CLI feature. CLI **0.218.0** is
a different, much older entry: *"Declarative Automation Bundles is generally available â€” April 23, 2024"*. Do not
...
- The CLI verb is still `databricks bundle â€¦` and the file is still `databricks.yml`. Nothing in code changes.
- On first mention, write "Declarative Automation Bundles (formerly Databricks Asset Bundles)". The community and
  virtually all blog content, Stack Overflow answers, and conference talks still say "DABs" â€” one parenthetical
...
- **Do not** confuse this with **Lakeflow**, which is a separate 2025 product umbrella (Lakeflow Connect, Spark
  Declarative Pipelines â€” formerly DLT, and Lakeflow Jobs â€” formerly Workflows). Lakeflow is *what you deploy*;
  bundles are *how you deploy it*.

---

## 2. Bundle mechanics â€” `databricks.yml`
...
- **jobs** â€” yes (`job`).
- **pipelines** â€” yes (`pipeline`, i.e. Spark Declarative Pipelines on Lakeflow).
- **model serving endpoints** â€” yes (`model_serving_endpoint`). Note the `run_as` incompatibility in Â§5.3.
- **schemas / volumes** â€” yes (`schema`, `volume`), since CLI 0.225.0 and 0.236.0 respectively.
- **apps** â€” yes (`app`), since CLI 0.239.0 (Jan 2025).
- **dashboards** â€” yes (`dashboard`), AI/BI dashboards, since CLI 0.232.0.
- **SQL warehouses** â€” yes (`sql_warehouse`).
- **catalogs** â€” **yes** (`catalog`) â€” but see the engine caveat in Â§2.3: UC catalogs and external locations are
  supported **only on the direct deployment engine**, not the legacy Terraform engine.
- **grants** â€” **not** a standalone top-level resource type. UNVERIFIED whether grants can be expressed nested
  ...
  expressible, that is a genuine argument for a thin Terraform layer alongside bundles (see Â§7).
...
- **Semantic difference that will bite people** â€” quoted behaviour: on the Terraform engine, *removing a field from
...
Latest release: **v1.10.0**, released **29 Jul 2026** â€” <https://github.com/databricks/cli/releases>

Recent cadence is roughly weekly: v1.10.0 (29 Jul), v1.9.0 (22 Jul), v1.8.0 (15 Jul), v1.7.0 (09 Jul),
v1.6.0 (02 Jul), v1.5.0 (24 Jun).

The install docs state a **minimum supported version of 0.205.0**, but that floor is meaningless for us â€” we need
â‰¥1.3.0 for the direct engine.
...
|---|---|---|
| `validate` | "Validate bundle configuration files are syntactically correct" | â€” |
...
| `deployment` | `bind`, `unbind`, `migrate` | â€” |
...
| `schema` | "Display JSON Schema for the bundle configuration" | â€” |
...
both `plan` and `deploy` take `--select` to target individual resources â€” but the release note explicitly warns
...
1. **Compile and test** â€” "Triggered on a pull request or a commit to the main branch. Compile code and run unit
   tests. Output a versioned file, for example, `my-app-1.0.jar`."
2. **Upload artifacts** â€” store built files in Unity Catalog volumes or an external repo, keyed by Git commit hash
   or semver.
3. **Validate bundle** â€” "Run `databricks bundle validate` to ensure that the `databricks.yml` configuration is
   correct."
4. **Deploy bundle** â€” "Use `databricks bundle deploy` to deploy the bundle to a staging or production environment."

### 4.3 Is there an official GitHub Action? YES

**`databricks/setup-cli`** â€” <https://github.com/databricks/setup-cli> â€” described in the Databricks docs as
...
### 4.4 Auth in CI â€” both options, and which to prefer
...
**(a) OAuth M2M service principal secret** â€” used in the page's bundle-deployment examples, passing a token via the
...
- Secrets are created under Settings â†’ Identity and access â†’ Service principals â†’ Secrets â†’ Generate secret.
- **Max 5 secrets per service principal; max lifetime 730 days.** So this option carries a mandatory rotation
  obligation â€” at minimum a 2-year hard expiry, and realistically much shorter.
...
**(b) GitHub OIDC â†’ Databricks workload identity federation (no stored secret)** â€” this is the better option and it
...
Note that `DATABRICKS_CLIENT_ID` and `DATABRICKS_HOST` are **not secrets** â€” they are a service principal UUID and
...
OIDC is subject to the same restriction â€” `id-token: write` is not granted to fork PRs, so an OIDC-authenticated
...
Use **GitHub Environments** for `staging` and `prod` â€” this pairs with the Databricks federation policy's
...
**CAVEAT â€” UNVERIFIED, needs a spike:** I have assumed `databricks bundle validate` runs without workspace
...
### 5.2 Development mode â€” documented behaviour
...
collide. This is the documented answer to "per-developer isolated dev deployments" â€” we should not invent our own
prefixing scheme.

### 5.3 Production mode â€” validations
...
### 5.4 `presets` â€” the customisation escape hatch
...
- **Not supported for model serving endpoints** â€” "An error occurs if these resources are defined in a bundle where
...
â€” supported for alert, cluster_policy, cluster, dashboard, job, pipeline, warehouse, notification destinations,
and others).
...
1. **Unit tests** â€” "Keep business logic in importable `src/` modules and cover it with `pytest` or an equivalent
   framework. Run these on every pull request so failures block merges."
2. **Bundle validation** â€” "Run `bundle validate` locally. In CI, prefer `bundle deploy` to a non-production
   workspace to catch YAML and resource-mapping issues before production deploys."
3. **Integration tests in staging** â€” "After deploying to staging, run end-to-end jobs with completion checks and
...
Development workflow: trunk-based â€” develop in a dev workspace, short-lived feature branches, merge to `main`,
...
ephemeral-CI-target recipe is a genuine differentiator â€” but we must build and verify it ourselves rather than
...
### Terraform â€” `databricks/databricks` provider

VERIFIED: latest **v1.123.0**, released **2026-07-29** â€”
...
Its coverage is materially **broader** than bundles: account-level resources (`mws_*` â€” workspace provisioning,
...
than deploying a workload â€” creating the workspaces themselves, wiring the metastore, managing per-tenant catalogs
...
the REST API) is the better-fitting tool, since `grants` is not a bundle resource type (Â§2.2). Note the irony that
bundles are *moving away* from Terraform internally (Â§2.3), which decouples the two â€” a Terraform layer for
...
application workloads â€” jobs, pipelines, apps, dashboards, serving endpoints. Do not use Terraform to deploy jobs
and pipelines when bundles exist; you lose dev-mode isolation, `bundle run`, and the whole local dev loop.

### Pulumi â€” `pulumi-databricks`

VERIFIED: latest **v1.98.0**, published **3 July 2026** â€” <https://www.pulumi.com/registry/packages/databricks/>,
source <https://github.com/pulumi/pulumi-databricks>. It is a bridged provider over the Terraform provider, so
resource coverage tracks Terraform's closely, with the benefit of real languages â€” **including C#**, which is
...
Pulumi and is not deprecated â€” but Databricks no longer documents or supports the path.

**When to pick Pulumi:** essentially only if the consuming organisation has already standardised on Pulumi.
For Lakewright.NET I'd recommend against making it the documented default despite the C# appeal â€” an open-source
...
(Jul 29, 2026), and examples (Jul 8, 2026) pages. Every key used below appears in the verified key lists in Â§2.1.

**Not deployed against a live workspace** â€” no Databricks workspace was available in this research session. Treat
as schema-correct-by-construction; validate with `databricks bundle validate` before publishing.

```yaml
# databricks.yml â€” Lakewright.NET
bundle:
  name: lakewright
...
- Because `run_as` is set, do **not** put a `model_serving_endpoint` in this bundle (Â§5.3) â€” it will error. Either
...
- The `ci` target's `name_prefix` preset needs `ci_run_id` declared as a variable and supplied via
  ...
  `presets.name_prefix` value â€” confirm during the spike; the fallback is `--var` at deploy time or a literal.
...
1. **`bundle validate` without credentials** â€” does it work? Determines whether fork PRs can run it (Â§4.5).
2. **Grants in bundles** â€” can grants be expressed nested in `catalog`/`schema`/`volume`, or is Terraform required
   for tenant grant management? Load-bearing for the multi-tenancy design (Â§2.2, Â§7).
3. **Variable substitution inside `presets.name_prefix`** â€” legal or not (Â§8).
4. **Ephemeral CI target + `bundle destroy`** â€” build and verify the recipe end to end; it is undocumented (Â§6).
5. **Direct-engine field-removal semantics** â€” confirm the "removed field reverts to default" behaviour against a
   real resource before we advertise the engine pin (Â§2.3).
...
| 3 | <https://docs.databricks.com/aws/en/dev-tools/bundles/faqs> | â€” |
...
| 15 | <https://github.com/databricks/setup-cli> | â€” |
...
| 22 | <https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows> | â€” |
...
- `docs.databricks.com/aws/en/dev-tools/bundles/targets` â†’ 404 (content lives in `reference`)
- `docs.databricks.com/aws/en/dev-tools/ci-cd/best-practices` â†’ 404 on the AWS path
- `learn.microsoft.com/en-us/azure/databricks/dev-tools/ci-cd/best-practices` â†’ 404
- `docs.azure.cn/en-us/databricks/dev-tools/ci-cd/best-practices` â†’ 404
