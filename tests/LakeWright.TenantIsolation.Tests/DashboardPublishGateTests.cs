...
/// <summary>
/// Tests for marker linting — author feedback that locates executable uses of
/// <c>__aibi_external_value</c> without asserting tenant isolation.
/// </summary>
/// <remarks>
/// <para>
...
/// bypass rode through and the cases the marker lint still has to handle correctly.
/// </para>
/// <para>
/// The production-shape reproduction is the most important test: a board whose WHERE clause
/// mentions the marker only inside a string literal used to pass a naive marker check. The lint
/// must refuse it, but its result remains author feedback rather than isolation proof.
/// </para>
/// </remarks>
[Trait("Category", "TenantIsolation")]
public class DashboardMarkerLintTests
{
    [Fact]
    public void A_dashboard_whose_dataset_filters_on_the_marker_passes()
    {
        ...
        var verdict = DashboardMarkerLint.Inspect(sql);
    ...
    {
        DashboardMarkerLint.Inspect(null!).Passed.ShouldBeFalse();
        DashboardMarkerLint.Inspect("").Passed.ShouldBeFalse();
        DashboardMarkerLint.Inspect("   \n\t  ").Passed.ShouldBeFalse();
    }
    ...
    public void Inspect_all_requires_every_dataset_to_pass()
    {
        ...
        var verdict = DashboardMarkerLint.InspectAll(datasets);
    ...
    {
        var verdict = DashboardMarkerLint.InspectAll(Array.Empty<string>());
    ...
    public void Inspect_dashboard_reports_the_name_of_each_unscoped_dataset()
    {
        ...
        var verdict = DashboardMarkerLint.InspectDashboard(serializedDashboard);
    ...
    {
        var verdict = DashboardMarkerLint.InspectDashboard("""{"datasets": []}""");
    ...
    {
        var verdict = DashboardMarkerLint.InspectDashboard("""{ "datasets": [{ "queryLines": "SELECT 1" }] }""");
