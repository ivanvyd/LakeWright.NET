...
class ConfidentialIdentifierScannerTests(unittest.TestCase):
    ...
    def test_workflows_cover_public_metadata_and_preflight_execution(self) -> None:
        ...
        self.assertIn('git cat-file -e "$BEFORE_SHA^{commit}"', ci)
        self.assertIn('CONFIDENTIAL_GIT_RANGE="HEAD"', ci)
        self.assertNotIn('CONFIDENTIAL_GIT_RANGE="HEAD^..HEAD"', ci)
        ...
        self.assertIn('symbols="${primary%.nupkg}.snupkg"', release)
        self.assertIn('test -f "$primary" && test -f "$symbols"', release)
        self.assertNotIn('dotnet nuget push "$symbols"', release)
